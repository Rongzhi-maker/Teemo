 ...
 * Description: com.yilan.sdk.common.ui
 */
public abstract class YLRecycleAdapter<D> extends RecyclerView.Adapter<BaseViewHolder<D>> {
    private List<D> dataList;
    private RecyclerView recyclerView;
    private ViewAttachedToWindowListener<D> viewAttachedToWindowListener;
    ...
    private int head = -0x000001 + this.hashCode();
    private int normal = 0x000001 + this.hashCode();
    private int foot = -0x000003 + this.hashCode();
    private int preLoadNumber = 4;
    private YLMultiRecycleAdapter multiRecycleAdapter;

    public Class getTypeClass() {
        Type type = this.getClass().getGenericSuperclass();
        while (type instanceof Class && type != YLRecycleAdapter.class) {
    ...
    public YLRecycleAdapter<D> longClickListener(OnItemClickListener<D> longClickListener) {
        this.longClickListener = longClickListener;
        return this;
    }

    public YLRecycleAdapter<D> multiClickListener(OnRecycleGestureListener<D> clickListener) {
        this.multiClickListener = clickListener;
        return this;
    }

    public YLRecycleAdapter<D> viewAttachListener(ViewAttachedToWindowListener<D> listener) {
    ...
    public BaseViewHolder<D> onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == head && headCreator != null) {
            ...
            if (longClickListener != null) {
                holder.setOnLongClick(longClickListener);
            }
    ...
    public void onViewAttachedToWindow(@NonNull BaseViewHolder<D> holder) {
        ...
        if (position >= 0) holder.setViewHolderPosition(position);
